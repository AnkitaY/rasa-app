...
2026-05-11 backend-engineer: S02 Stage 2 schema migration → done | 20260511000001_s02_schema_additions.sql applied to prod; added meals_meal_type_check constraint; added raw_text/source/recipe_type/prep_friendly/assembly_time_mins/excluded_from_plans to recipes; added meal_types_default/meal_days_default/meal_prefs/health_goals to user_preferences; lib/types.ts updated; npm run build ✓ (46/46 pages)
2026-05-11 backend-engineer: FEAT-002a POST /api/recipes/import → done | rewrote old Phase 0 URL-scraping+AI route; raw save only, no AI calls; validates anon_id/name/raw_text/recipe_type; meal_type[] stored as first element (DB col is text); GET /api/recipes/list updated to return raw_text/source/recipe_type/source_url; npm run build ✓ (46/46 pages)
